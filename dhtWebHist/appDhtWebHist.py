...
def getInternetData():
	page = requests.get('https://pogoda.onet.pl/prognoza-pogody/poznan-335979')
	soup = BeautifulSoup(page.text, 'html.parser')
	temp_box = soup.find('div', attrs={'class': 'temp'})
	hum_box = soup.find('div', attrs={'class': 'weatherParams'})
	templn = temp_box.text.strip()
	params = hum_box.text.strip()
	humln = params.split("\n")
	tempOut = templn.replace('°', '')
	humOut = humln[17].replace('%', '')
